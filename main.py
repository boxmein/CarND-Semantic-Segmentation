...
def load_vgg(sess, vgg_path):
    ...
    image_input = model.get_tensor_by_name(vgg_input_tensor_name)
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Huge upsample
    out = tf.layers.conv2d_transpose(out, num_classes, 16, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    print("training size: ", nn_last_layer.get_shape())
    print("correct size: ", correct_label.get_shape())

    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    corrects = tf.reshape(correct_label, (-1, num_classes))
    xent_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=correct_label)) 
...
def run():
    ...
    with tf.Session() as sess:
        ...
        final_layer = layers(l3_out, l4_out, l7_out, num_classes)
        correct_label = tf.placeholder(tf.int32, [None, None, None, num_classes], name='correct_label')
        learning_rate = tf.placeholder(tf.float32, name='learning_rate')
        logits, optimizer, xent_loss = optimize(final_layer, correct_label, learning_rate, num_classes)
        
        # TODO: Train NN using the train_nn function

        train_nn(sess, epochs, batch_size, get_batches_fn, optimizer, xent_loss, input_image, correct_label, keep_prob, learning_rate)
